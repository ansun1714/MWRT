...
wh3000|wh3000pro)
    ...
    # ════════════════════════════════════════════════════════
    # ★ WiFi 核心修复：补充缺失的 netifd-wireless.sh
    #
    # 根因（ChatGPT SSH 排查确认）：
    #   LEDE master 正处于 WiFi 脚本架构迁移阶段：
    #   · mac80211.sh 已更新，首行调用：
    #     . /lib/netifd/netifd-wireless.sh
    #   · 但 LEDE 的 wifi-scripts 包尚未将 netifd-wireless.sh 打包进固件
    #   · 结果：WiFi 驱动初始化入口缺失，所有无线功能完全瘫痪
    #
    # 修法：通过 files/ 机制把 netifd-wireless.sh 直接打包进固件
    #       不动 wifi-scripts 的 Makefile，不删任何原有文件
    #       mac80211.sh 由 wifi-scripts 包正常安装，补充缺失的那块拼图
    #
    # 为何 ChatGPT 的方案导致编译报错：
    #   它 rm -rf 整个 wifi-scripts 再重建，丢失了 mac80211.sh 等文件，
    #   新 Makefile 里的 DEPENDS 在 LEDE 上不存在，编译依赖检查失败
    # ════════════════════════════════════════════════════════
    ...
    # ── WiFi：预置静态配置（MT7981 路径固定，最可靠方案）────
    # mac80211.sh 识别到 netifd-wireless.sh 后，会正确读取这份配置
