...
# ============================================================
# Fix-WIFI-v2：修复 wifi-scripts 与 netifd 的文件冲突
#
# DEBUG-30 实际错误：
#   Package wifi-scripts wants to install
...
#
# 取消独立 wifi-scripts 包，避免与 netifd 重复提供同一文件。
# ============================================================
echo ">>> [Fix-WIFI-v2] 修复 wifi-scripts / netifd 文件冲突..."

# 不让 wifi-scripts 作为独立 IPK 安装
sed -i \
    -e '/^CONFIG_PACKAGE_wifi-scripts=/d' \
...
# 如果 feeds 中存在 wifi-scripts 包目录，也不修改其源码。
# 只取消它的选包状态，避免破坏已经验证有效的 WiFi 文件。
echo ">>> [Fix-WIFI-v2] 已取消 CONFIG_PACKAGE_wifi-scripts"
echo ">>> [Fix-WIFI-v2] 保留 files/lib/netifd/netifd-wireless.sh"


echo ">>> [Fix-WIFI-v2] 最终检查..."
if grep -q '^CONFIG_PACKAGE_wifi-scripts=y' .config; then
    echo "❌ ERROR：wifi-scripts 仍被选中"
    exit 1
fi

if [ -f files/lib/netifd/netifd-wireless.sh ]; then
    echo ">>> [OK] 手动补齐的 netifd-wireless.sh 保留"
else
    echo "❌ ERROR：files/lib/netifd/netifd-wireless.sh 不存在"
    exit 1
fi
